...
public class SQLControllerMth1 {
	...
	public void InsertCluster(ArrayList<Document> cluster) {
		...
		try {
			for (int i = 0; i < cluster.size(); i++) {
				...
				ps = conn.prepareStatement("INSERT INTO mahoutldaclustermonth1tfidf (idData, TopicID) VALUES (?,?)");
	...
	public void InsertTopic(ArrayList<Topic> topicdata) {
		...
		try {
			for (int i = 0; i < topicdata.size(); i++) {
				...
				ps = conn.prepareStatement("INSERT INTO mahoutldatopicmonth1tfidf (TopicID, ClusterTitle, Score) VALUES (?,?,?)");
	...
	public ArrayList<Double> IndexCount(int numtopics) {
		...
		try {
			for (int i = 0; i < numtopics; i++) {
				...
				ps = conn.prepareStatement("Select count(*) From mahoutldaclustermonth1tfidf Where TopicID = " + i);
	...
	public void GetdataId(ArrayList<String> dataid) {
		...
		try {
				...
				ps = conn.prepareStatement("Select idData from mahoutldaclustermonth1tfidf");
