...
public class SQLControllerMth6 {
	...
	public void InsertCluster(ArrayList<Document> cluster) {
		...
		try {
			for (int i = 0; i < cluster.size(); i++) {
				...
				ps = conn.prepareStatement("INSERT INTO mahoutldaclustermonth6tfidf (idData, TopicID) VALUES (?,?)");
	...
	public void InsertTopic(ArrayList<Topic> topicdata) {
		...
		try {
			for (int i = 0; i < topicdata.size(); i++) {
				...
				ps = conn.prepareStatement("INSERT INTO mahoutldatopicmonth6tfidf (TopicID, ClusterTitle, Score) VALUES (?,?,?)");
	...
	public ArrayList<Double> IndexCount(int numtopics) {
		...
		try {
			for (int i = 0; i < numtopics; i++) {
				...
				ps = conn.prepareStatement("Select count(*) From mahoutldaclustermonth6tfidf Where TopicID = " + i);
	...
	public void GetdataId(ArrayList<String> dataid) {
		...
		try {
				...
				ps = conn.prepareStatement("Select idData from mahoutldaclustermonth6tfidf");
